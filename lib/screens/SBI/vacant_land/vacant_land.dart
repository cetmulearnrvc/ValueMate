...
import 'package:login_screen/screens/SBI/vacant_land/savedDraftsSBIVacantland.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const SBIVacantLandFormPage(propertyData: {}),
    );
  }
}

class SBIVacantLandFormPage extends StatefulWidget {
  // const VacantLandFormPage({super.key});
  final Map<String, dynamic>? propertyData;
  const SBIVacantLandFormPage({super.key, this.propertyData});

  @override
  State<SBIVacantLandFormPage> createState() => _ValuationFormPageState();
}

class _ValuationFormPageState extends State<SBIVacantLandFormPage> {
  @override
  void initState() {
    super.initState();
  ...
  List<pw.TableRow> _getPage2Table5Rows() {
    ...
    rows.add(pw.TableRow(children: [
      pw.Container(
          padding: cellPadding, child: pw.Text('18.', style: contentTextStyle)),
      pw.Container(
          padding: cellPadding,
    ...
    rows.add(pw.TableRow(children: [
      ...
      pw.Container(
          padding: cellPadding,
          child: pw.Text('Type of Demarcation (Side stone/Fencing)',
              style: contentTextStyle)),
      pw.Container(
          padding: cellPadding, child: pw.Text(':', style: contentTextStyle)),
      pw.Container(
  ...
  Future<void> _generatePdf() async {
    ...
    // below code were commented for changing to text field
    ...
    final List<pw.TableRow> page2Table3Rows =
        _getPage2Table3Rows(); // Get rows for section 18
                  ...
                  children: [
                    pw.Column(
                      ...
                      children: [
                        ...
                        pw.Text('The State Bank of India',
                            style: const pw.TextStyle(fontSize: 12)),
  ...
  Future<void> _saveToNearbyCollection() async {
    try {
      ...
      final dataToSave = {
        ...
        'bankName': 'State Bank of India (Vacant Land)',
      };
      ...
      try {
        ...
        if (_dateOfInspectionController.text.isNotEmpty) {
          final inspectionDate =
              DateFormat('dd-MM-yyyy').parse(_dateOfInspectionController.text);
        ...
        if (_dateOfValuationController.text.isNotEmpty) {
          final valuationDate =
              DateFormat('dd-MM-yyyy').parse(_dateOfValuationController.text);
          request.fields['dateOfValuation'] =
              DateFormat('yyyy-MM-dd').format(valuationDate);
        }
        ...
        if (response.statusCode == 200 || response.statusCode == 201) {
          ...
          if (context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                  content:
                      Text(errorJson['message'] ?? 'Failed to save valuation')),
            );
          }
        }
      } catch (e) {
                  ...
                  child: FloatingActionButton.extended(
                      ...
                      onPressed: () {
                        Navigator.of(context).push(
                          MaterialPageRoute(
                            builder: (ctx) {
                              return const SavedDraftsSBIVacantLand();
                            },
                          ),
                        );
                      }),
              ...
              ExpansionTile(
                ...
                children: <Widget>[
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      labelText:
                          '17. Type of road available at present (Bitumen/Mud/CC/Private)',
